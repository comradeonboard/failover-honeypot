...
- Hosts (user-added): the FIRST device to log in is the MAIN SERVER permanently (hosts table in honeypot.db, survives restarts); later devices register as Second Hosts. Frontend stores a device_id in localStorage ('fhm_device_id') and sends it with login; backend parses OS/device/browser from the User-Agent. GET /api/auth/hosts; online status computed server-side (last_seen within 90s; middleware touches last_seen max once per 30s).
- Defense shield (user-added, backend DefenseShield class): strike tracking (3 strikes/5 min → 1h ban, 2nd offence permanent), flood control (15 conns/min → ban), persistent blocklist (blocked_ips + ban_history tables in honeypot.db), banned IPs silently dropped by every honeypot handler. APIs: GET /api/defense/status, POST /api/defense/ban {ip, reason}, POST /api/defense/unban {ip}, GET /api/defense/export (iptables rules). Assessed via defense.assess_alert() on every honeypot alert.
